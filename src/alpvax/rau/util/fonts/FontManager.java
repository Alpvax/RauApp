...
public class FontManager
{
	...
	public FontManager(String... directories)
	{
		if(available())
		{
			...
			{
				File dir = new File(pathToExternalStorage + "/rau/fonts/" + (s.startsWith("/") ? s.substring(1) : s));   
				// have the object build the directory structure, if needed.
				dir.mkdirs();
				fontDirs.add(dir.getAbsolutePath());
			}
		}
	}
	private boolean available()
	{
		if(available == null)
		{
			...
			available = Boolean.valueOf(Environment.MEDIA_MOUNTED.equals(state) || Environment.MEDIA_MOUNTED_READ_ONLY.equals(state));
		}
		return available.booleanValue();
	}
	public FontManager useSystemFonts()
	{
